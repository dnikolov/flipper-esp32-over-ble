...
/* docs/PLAN.md Phase 8 step 3: base BLE transport/pairing/session-auth layer ported from
   esp32/main/main.c (this board's primary porting reference: RISC-V + NimBLE central, same
   as the C6, unlike the Heltec's classic Xtensa) onto the OLIMEX MOD-ESP32-C5's NimBLE
   central role, reusing components/feb_protocol/ unchanged. Any wire-format/crypto logic
   below must stay byte-for-byte identical to the C6/Heltec builds and docs/PROTOCOL.md -- see
   esp32/main/main.c for the equivalent implementation and its own inline rationale, not
   repeated here where behavior is unchanged.

   Phase 8 capability-porting pass: `wifi_scan`, `ble_scan`, and `gps` ported from
   esp32/main/main.c, manual-scan/poll-only paths. **Step 3 (2026-09-25 scope reversal):**
   `wifi_scan`/`wardriving` now scan both 2.4GHz and 5GHz (see start_wifi_subsystem()'s comment)
   and `wardriving` is now ported too (ported from esp32/main/main.c's reference, structurally
   identical to the Heltec's port) -- both restrictions from the original step-2 pass are
   lifted. feb_features[] and handle_command()'s dispatch reflect all four implemented
   capabilities. This board also has no onboard pushbutton
   (docs/hardware/olimex-mod-esp32-c5/README.md), so factory_reset.c is not ported either --
   docs/BACKLOG.md BL15 tracks this as a deliberate scope cut, not an oversight;
   feb_wipe_pairing_secrets() below is kept as dead code for forward compatibility with whatever
   unpair/reset mechanism eventually lands, not currently reachable from anywhere. */
...
/* Same fixed 64-byte Flipper write-characteristic cap as the C6/Heltec builds (flipper/
...
/* `wifi_scan`/`ble_scan` capability constants, ported from esp32/main/main.c unchanged -- see
   that file's fuller comments (same rationale applies verbatim; this board's
...
/* `wardriving` capability, ported from esp32/main/main.c unchanged -- see that file's fuller
   comments (docs/PLAN.md "`ble_scan`, `wardriving`, and the GPS-stub reorder"; docs/PROTOCOL.md
   "`wardriving` command and status payloads"). Interval bounds/defaults live in
...
typedef enum {
    ...
    TX_DONE_CONTINUE_WARDRIVING, /* wardriving_send_next_batch()'s own re-entry point -- see
                                     esp32/main/main.c's fuller comment. */
...
/* docs/PLAN.md "`ble_scan`, `wardriving`, and the GPS-stub reorder": wardriving's Wi-Fi/BLE
   ...
   rather than a second parallel set of state -- see esp32/main/main.c's fuller comment.
   Ported unchanged. */
typedef enum {
...
/* wardriving's own start/stop state and configured cadence, independent of BLE connection
   ...
   on-device flash log"). Ported unchanged from esp32/main/main.c. */
...
/* Wi-Fi scan band selection (docs/PROTOCOL.md's `wifi_band` row, added 2026-09-26) -- this
   board's own dual-band (2.4GHz+5GHz) radio is the only reason this field has any observable
   effect here; the C6/Heltec decode-and-ignore it (see cbor_wardriving.c). Unlike
   wardriving_country_t/esp_wifi_set_country_code() and esp_wifi_set_band_mode() themselves
   (both persistent radio-level settings that stay in effect until next changed),
   ...
   call, so wardriving_wifi_band itself must be remembered in software (read back by
   wardriving_apply_wifi_band() on every wardriving re-arm and by handle_wifi_scan_command()'s
   manual scan) the same way wardriving_wifi_swelling already is. */
typedef enum {
    WARDRIVING_BAND_2_4GHZ = 0,
    WARDRIVING_BAND_5GHZ_FAST = 1,
    WARDRIVING_BAND_5GHZ_FULL = 2,
} wardriving_wifi_band_t;

static wardriving_swelling_mode_t wardriving_wifi_swelling;
static bool wardriving_swelling_aggressive_active;
/* Boot default matches this board's pre-existing (2026-09-25, "Scope reversal") behavior --
   band_mode WIFI_BAND_MODE_AUTO set once in start_wifi_subsystem(), no channel_bitmap
   restriction -- so a manual wifi_scan run before wardriving ever starts sees the same
   dual-band full-sweep behavior it already did before this field existed. */
static wardriving_wifi_band_t wardriving_wifi_band = WARDRIVING_BAND_5GHZ_FULL;
...
/* Last-saved on/off + Wi-Fi/BLE settings (wardriving_persist.h), loaded once at boot and
   kept in sync with every start/stop transition -- source of truth for the boot autostart.
   Unlike the C6/Heltec builds, this board has no onboard pushbutton (docs/BACKLOG.md BL15),
   so there is no boot-button-toggle path reading/writing this -- boot autostart and the
   `wardriving` command's start/stop actions are the only writers. */
...
/* Set the first time host_synced() runs its autostart-from-persisted-state block, so a
   ...
   host_synced()'s comment. Ported unchanged from esp32/main/main.c. */
static bool wardriving_autostart_attempted;
/* True while a wardriving status(state="data") batch's fragments are still going out --
   ported unchanged from esp32/main/main.c; see that file's comment on the 2026-09-10
   GATT-write-flood bug this flag's exact clear-timing fixes. */
...
/* Count of records included in the batch currently in flight, marked drained from the flash
   log only on the next wardriving_send_next_batch() re-entry. Ported unchanged. */
...
/* Number of valid entries in wifi_scan_raw_records[] from the scan that just completed --
   set by wifi_scan_done_handler() (sys_evt task) right before it hands off to
   wifi_scan_done_cb() (NimBLE host task) via wifi_scan_done_co, needed so the
   wardriving-source dedup/append loop (which must run on the NimBLE host task, not sys_evt --
   see wifi_scan_done_cb()'s comment) has the count carried across that handoff. */
...
static void wardriving_set_wifi_band_mode(wardriving_wifi_band_t band);
...
static bool wardriving_start_internal(bool want_wifi, bool want_ble, bool want_ble_passive,
                                      ...
                                      wardriving_wifi_band_t wifi_band);
...
static void compute_board_id(void)
{
    ...
    if (err != ESP_OK) {
        ...
        memcpy(board_id_buf, "esp32c5-unknown", sizeof("esp32c5-unknown"));
        board_id_len = strlen(board_id_buf);
        return;
    }
    written = snprintf(board_id_buf, sizeof(board_id_buf), "esp32c5-%02x%02x%02x%02x%02x%02x",
...
/* docs/BACKLOG.md BL15: this board has no onboard pushbutton, so no factory_reset.c exists
   here and nothing calls this today -- kept only so a future unpair/reset mechanism (whatever
   form it takes on a board with no boot button) has a ready-made zeroization entry point,
   matching the C6/Heltec's feb_wipe_pairing_secrets() contract byte-for-byte. */
...
/* docs/PROTOCOL.md "`wifi_scan` command and status payloads": highest-generation PHY string
   ...
   phy_11ax can legitimately be set on a real scan here (this chip is Wi-Fi 6) on either band,
   since start_wifi_subsystem() now scans both 2.4GHz and 5GHz (2026-09-25 scope reversal). */
...
static void wifi_scan_done_handler(void *arg, esp_event_base_t base, int32_t id, void *data)
{
    ...
    if (wifi_scan_active_source == WIFI_SCAN_SOURCE_WARDRIVING) {
        /* The wardriving dedup/append loop must run on the NimBLE host task (the same task
           that reads the wardriving log via wardriving_send_next_batch()), not here on
           sys_evt -- see esp32/main/main.c's G30 fix comment. This handler's only remaining
           job for the wardriving source is to hand the raw scan results across that task
           boundary, same shape as the non-wardriving branch below already uses
           wifi_scan_done_co for. */
...
/* Runs on the NimBLE host task (wifi_scan_done_co's queue). Ported unchanged from
   esp32/main/main.c's manual-scan path, plus the wardriving-source dedup/append branch. */
static void wifi_scan_done_cb(struct ble_npl_event *ev)
{
    (void)ev;
...
static void handle_wifi_scan_command(uint16_t conn_handle, const feb_command_payload_t *cmd)
{
    ...
    /* docs/PROTOCOL.md's `wifi_band` row: "a later manual wifi_scan observes whatever
       wifi_band wardriving last set" -- wardriving_wifi_band is that sticky setting
...
static void ble_scan_window_close_cb(struct ble_npl_event *ev)
{
    ...
    if (ble_scan_active_source == BLE_SCAN_SOURCE_WARDRIVING) {
        /* wardriving logs every cataloged device this window (up to the same
           ...
           is specific to ble_scan's one-shot reporting contract. Ported unchanged from
           esp32/main/main.c. */
        feb_location_t fix;
        feb_location_state_t loc_state = location_get_fix(&fix);
        uint16_t k;
...
/* docs/PLAN.md "Real GPS driver, wardriving fix-dependency, and real wardriving-record
   ...
   unchanged from esp32/main/main.c, pins retargeted to GPIO4/GPIO5 in location.c only. */
static void handle_gps_command(uint16_t conn_handle, const feb_command_payload_t *cmd)
{
    size_t arg_count;
    feb_cbor_status_t status;
    ...
    /* Same 192-byte sizing as esp32/main/main.c -- see that file's comment on the 2026-09-21
       hardware-verified undersizing bug (real lat/lon/timestamp/altitude values need their
       full uint encoding on every genuine fix, not just in a theoretical worst case). */
...
/* docs/PLAN.md "`ble_scan`, `wardriving`, and the GPS-stub reorder": capability-name
   dispatch. Ported unchanged from esp32/main/main.c. */
static void handle_command(uint16_t conn_handle, const feb_command_payload_t *cmd)
{
...
/* docs/PLAN.md "`ble_scan`, `wardriving`, and the GPS-stub reorder" / docs/PROTOCOL.md
   ...
   (wardriving_wifi_interval_co's queue). Ported unchanged from esp32/main/main.c, including
   docs/WARDRIVING_REDESIGN.md's speed-based swelling hysteresis. */
static void wardriving_wifi_interval_cb(struct ble_npl_event *ev)
{
    wifi_scan_config_t scan_cfg;
    esp_err_t err;
...
/* Same re-arming role as wardriving_wifi_interval_cb() above, for the BLE source. Ported
   unchanged from esp32/main/main.c. */
static void wardriving_ble_interval_cb(struct ble_npl_event *ev)
{
    struct ble_gap_disc_params params = {0};
    int rc;
...
/* Shared teardown for every wardriving stop path (explicit `stop` command, self-stop on
   error). Unlike the C6/Heltec builds, there is no boot-button toggle-off path here (no
   onboard pushbutton -- docs/BACKLOG.md BL15). Ported unchanged otherwise from
   esp32/main/main.c. Runs on the NimBLE host task only. */
static void wardriving_stop_internal(void)
{
    if (wardriving_wifi_active) {
...
/* Sets scan_cfg->scan_time.active.min/max per the currently-stored WiFi swelling mode
   (docs/WARDRIVING_REDESIGN.md). Ported unchanged from esp32/main/main.c. */
static void wardriving_apply_wifi_swelling(wifi_scan_config_t *scan_cfg)
{
...
/* Sets scan_cfg->channel_bitmap per the currently-stored Wi-Fi band mode
   (docs/PROTOCOL.md's `wifi_band` row) -- only WARDRIVING_BAND_5GHZ_FAST needs a per-call
   channel_bitmap; WARDRIVING_BAND_2_4GHZ/WARDRIVING_BAND_5GHZ_FULL rely entirely on the
   radio-level band_mode already set by esp_wifi_set_band_mode() (2G_ONLY / AUTO respectively)
   and leave channel_bitmap zeroed for a normal full sweep of whatever band(s) that mode
   allows. `channel` must stay 0 for channel_bitmap to take effect at all (confirmed against
   esp_wifi_types_generic.h's wifi_scan_config_t.channel_bitmap comment and
   esp-idf/examples/wifi/scan/main/scan.c's array_2_channel_bitmap() usage) -- scan_cfg is
   always freshly memset(0) by every caller of this function, so that's already satisfied.
   The non-DFS 5GHz set (36/40/44/48 UNII-1-low, 149/153/157/161/165 UNII-3) matches
   docs/PROTOCOL.md's `wifi_band` row exactly -- see wifi_5g_channel_bit_t in
   esp_wifi_types_generic.h for the channel->bit mapping. */
static void wardriving_apply_wifi_band(wifi_scan_config_t *scan_cfg)
{
    if (wardriving_wifi_band != WARDRIVING_BAND_5GHZ_FAST) {
        return;
    }
    scan_cfg->channel = 0;
...
/* Sets the radio-level band mode for the given wifi_band selection (docs/PROTOCOL.md's
   `wifi_band` row) -- persistent until next changed, same "global radio setting" semantics as
   esp_wifi_set_country_code() right next to this call's usual call site. WARDRIVING_BAND_
   5GHZ_FAST also needs WIFI_BAND_MODE_AUTO (both bands enabled) since its restriction to
   non-DFS channels is expressed via wardriving_apply_wifi_band()'s per-call channel_bitmap,
   not via band_mode itself. */
static void wardriving_set_wifi_band_mode(wardriving_wifi_band_t band)
{
    wifi_band_mode_t mode = (band == WARDRIVING_BAND_2_4GHZ) ? WIFI_BAND_MODE_2G_ONLY : WIFI_BAND_MODE_AUTO;
    ...
    wardriving_wifi_band = band;
}

/* Shared start path for every wardriving start trigger (explicit `start` command; boot
   autostart from persisted state). Unlike the C6/Heltec builds, there is no boot-button
   toggle-on trigger here. Ported unchanged otherwise from esp32/main/main.c. Rolls back
   atomically on a partial failure. Runs on the NimBLE host task only. */
static bool wardriving_start_internal(bool want_wifi, bool want_ble, bool want_ble_passive,
                                      uint32_t wifi_interval_ms, uint32_t ble_window_ms,
                                      uint32_t ble_interval_ms,
                                      wardriving_swelling_mode_t wifi_swelling,
                                      ...
                                      wardriving_wifi_band_t wifi_band)
{
    ...
    if (want_wifi) {
        ...
        wardriving_set_wifi_band_mode(wifi_band);
...
/* Stops whichever wardriving source(s) are active and, if connected+authenticated, sends
   the docs/PROTOCOL.md-specified unsolicited error + status(state="stopped") pair. Ported
   unchanged from esp32/main/main.c. Runs on the NimBLE host task only. */
static void wardriving_self_stop(const char *error_code)
{
...
/* docs/PROTOCOL.md "`wardriving` command and status payloads" / docs/CAPABILITIES.md's
   ...
   comment for the "required vs. default when omitted" interpretation note. */
static void handle_wardriving_command(uint16_t conn_handle, const feb_command_payload_t *cmd)
{
    ...
    wardriving_wifi_band_t wifi_band = WARDRIVING_BAND_2_4GHZ;
    size_t i;
    ...
    if (is_status_query) {
        if (payload.has_sources || payload.has_wifi_interval_ms || payload.has_ble_params ||
            payload.has_wifi_swelling || payload.has_country || payload.has_wifi_band) {
            if (!send_protected_error(conn_handle, "invalid_command", strlen("invalid_command"), 1, cmd->request_id)) {
                ble_gap_terminate(conn_handle, BLE_ERR_REM_USER_CONN_TERM);
            }
            return;
    ...
    if (want_wifi) {
        ...
        if (!payload.has_wifi_swelling || !payload.has_country || !payload.has_wifi_band) {
            if (!send_protected_error(conn_handle, "invalid_command", strlen("invalid_command"), 1, cmd->request_id)) {
                ble_gap_terminate(conn_handle, BLE_ERR_REM_USER_CONN_TERM);
            }
            return;
        ...
        if (payload.wifi_band_len == strlen("2.4ghz") &&
            ...
            wifi_band = WARDRIVING_BAND_2_4GHZ;
            wifi_band_ok = true;
        } else if (payload.wifi_band_len == strlen("5ghz_fast") &&
                  memcmp(payload.wifi_band, "5ghz_fast", payload.wifi_band_len) == 0) {
            wifi_band = WARDRIVING_BAND_5GHZ_FAST;
            wifi_band_ok = true;
        } else if (payload.wifi_band_len == strlen("5ghz_full") &&
                  memcmp(payload.wifi_band, "5ghz_full", payload.wifi_band_len) == 0) {
            wifi_band = WARDRIVING_BAND_5GHZ_FULL;
            wifi_band_ok = true;
        }

        if (!wifi_swelling_ok || !country_ok || !wifi_band_ok) {
            if (!send_protected_error(conn_handle, "invalid_command", strlen("invalid_command"), 1, cmd->request_id)) {
                ble_gap_terminate(conn_handle, BLE_ERR_REM_USER_CONN_TERM);
            }
            return;
        }
    } else if (payload.has_wifi_swelling || payload.has_country || payload.has_wifi_band) {
        if (!send_protected_error(conn_handle, "invalid_command", strlen("invalid_command"), 1, cmd->request_id)) {
            ble_gap_terminate(conn_handle, BLE_ERR_REM_USER_CONN_TERM);
        }
        return;
    ...
    if (!wardriving_start_internal(want_wifi, want_ble, want_ble_passive,
                                   ...
                                   wifi_swelling, country, wifi_band)) {
...
static int gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC_COMPLETE:
        /* Suppressed while wardriving's BLE source is active: that source re-arms its own
           ...
           reconnect scan alongside it. Ported unchanged from esp32/main/main.c. */
    ...
    case BLE_GAP_EVENT_DISCONNECT: {
        ...
        if (wifi_scan_in_progress && wifi_scan_active_source == WIFI_SCAN_SOURCE_MANUAL) {
            /* Don't clear wifi_scan_in_progress directly here -- the radio scan this
               ...
               capture (if active) must keep running across this disconnect (docs/PROTOCOL.md:
               "continues across BLE disconnects"), so wifi_scan_done_cb()'s WARDRIVING branch
               does not check connection state at all. Ported unchanged from
               esp32/main/main.c. */
            esp_err_t serr = esp_wifi_scan_stop();
        ...
        if (ble_scan_in_progress && ble_scan_active_source == BLE_SCAN_SOURCE_MANUAL) {
            /* Same disconnect-safety shape as wifi_scan_in_progress above, gated to the
...
/* docs/PROTOCOL.md "`wifi_scan` command and status payloads": esp_netif/default event
   ...
   5GHz (36+) channel numbers never overlap -- confirmed against the installed ESP-IDF v5.5.2
   headers that wifi_ap_record_t.primary ("Channel of AP") is a single band-agnostic channel
   number with no separate band field, so a 5GHz result's channel is reported through the exact
   same field 2.4GHz results already use, band already implicit. **2026-09-26 update**: a
   runtime band-switching command now exists after all -- docs/PROTOCOL.md's `wifi_band` row
   on wardriving's `start` action -- see wardriving_set_wifi_band_mode()/
   wardriving_apply_wifi_band(). The esp_wifi_set_band_mode(AUTO) call below remains this
   board's boot-time default (matching wardriving_wifi_band's own WARDRIVING_BAND_5GHZ_FULL
   default) until a wardriving `start` command's `wifi_band` field changes it.
   ...
   manual one-shot scan) -- see docs/BACKLOG.md BL16 and the wardriving port below. */
static void start_wifi_subsystem(void)
{
...
static void host_synced(void)
{
    ...
    /* docs/BACKLOG.md "Per-board wardriving autostart setting": resume before start_scan()
       ...
       source owns discovery" guard (wardriving_ble_active) already sees the right value on
    ...
    if (!wardriving_autostart_attempted) {
        ...
        if (wardriving_persisted.enabled) {
            /* wifi_band is not persisted (like wifi_swelling/country above it, autostart uses
               the conservative default rather than whatever a prior live `start` command last
               chose) -- WARDRIVING_BAND_2_4GHZ, this board's pre-dual-band default, since the
               2.4GHz+BLE coexistence combo is the only one with any prior verification and
               dual-band radio coexistence has never been swept (docs/BASELINES.md's
               MOD-ESP32-C5 entry) -- an unattended boot autostart is not the place to default
               to the untested combo. */

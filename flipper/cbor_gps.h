/* Shared contract, mirrored byte-for-byte in components/feb_protocol/cbor_gps.h (added
   there first, 2026-09-12 frozen design session, docs/PLAN.md "Real GPS driver, wardriving
   ...
   field list below was checked by hand against components/feb_protocol/cbor_gps.h, not just
   by a clean script run.
   ...
   fix_quality, satellites, hdop_e1, utc_timestamp_s, altitude_dm_offset, speed_e1_kmh --
   matches PROTOCOL.md exactly. `altitude_dm_offset` was appended after the original six
   fields (added 2026-09-12, same design session); `speed_e1_kmh` was appended after that
   (added 2026-09-21, docs/WARDRIVING_REDESIGN.md) rather than inserted next to
   lat/lon_e7_offset, since this codec's field order is meaningful (the decoder below rejects
   out-of-order keys) and appending is the minimal, additive change. Unlike wifi_scan/ble_scan/
...
typedef struct {
    ...
    uint64_t speed_e1_kmh; /* RMC ground speed (knots x 1.852), scaled by 10 and truncated,
                               same convention as hdop_e1 */

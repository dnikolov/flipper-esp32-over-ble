...
/* Splits `record` (record_len bytes, 1..FEB_MAX_RECORD_SIZE) into fragments of at most
   ...
   time; each fragment is assembled into a file-scope static buffer sized
   FEB_FRAG_HEADER_SIZE + FEB_MAX_RECORD_SIZE (not a caller-supplied buffer, and not
   dynamically allocated) — safe because fragmentation on both firmwares is synchronous
   and single-in-flight, never reentrant or concurrent. `emit`'s callback must copy out
   anything it needs to keep past the call, since the buffer is reused for the next
   fragment. Returns the fragment_count on success, or 0 if record_len is 0, exceeds
   FEB_MAX_RECORD_SIZE, capacity is 0, or the record would need more than
   FEB_MAX_FRAGMENTS fragments. */

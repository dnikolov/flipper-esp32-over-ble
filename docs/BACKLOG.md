...
| BL21 | `heltec/main/meshcore_radio.cpp`'s RX task carries temporary `ESP_LOGI` diagnostic logging (IRQ-fired frame length, raw hex + RSSI on successful `readData()`, `meshcore_proto_parse()` success/failure, `meshcore_table_upsert()` node_id) added 2026-09-26 to distinguish "no MeshCore RF traffic in range" from "a bug in the RX/parse pipeline is swallowing frames" | Open — remove once real-world MeshCore reception is confirmed working (or confirmed absent) via field observation |

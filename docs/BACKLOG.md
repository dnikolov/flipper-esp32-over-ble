...
| G30 | Wardriving log/dedup state has no lock between the Wi-Fi `sys_evt` writer and the NimBLE-host drain reader | **FIX APPLIED 2026-09-13, hardware re-verification pending.** Moved the Wi-Fi-source dedup/append loop out of `wifi_scan_done_handler()` (sys_evt task) into `wifi_scan_done_cb()` (NimBLE host task, via the existing `wifi_scan_done_co` handoff and a new `wifi_scan_raw_count` carrier variable) — now matches the BLE-source sibling (`ble_scan_window_close_cb()`), which was already same-task-safe. The wardriving log API is now single-threaded (NimBLE host task only) by construction; no mutex needed. Build-verified clean, all ESP32 host suites pass. **Needs a live test**: run wardriving with the Flipper actively draining backlog while new Wi-Fi results are captured, to confirm no more torn counters/corrupted pointers under load. |
...
| ID | Title | Status |
| --- | --- | --- |
| G12 | Flipper fragments every record at ATT MTU 23 (16-byte payload) even after MTU negotiation | **FIX APPLIED 2026-09-13, hardware re-verification pending.** Added `negotiated_att_mtu` tracking in `profile_event_handler()` (reads `ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE` directly off the same raw BLE event stream `event_dispatcher.c` delivers to every registered handler before Furi's own internal GAP handler — no public Furi API exposes this otherwise). `send_pairing_record()` now uses `feb_fragment_capacity(min(negotiated_att_mtu, FEB_NOTIFY_CHAR_EFFECTIVE_MTU))`, mirroring the ESP32's already-correct `FEB_FLIPPER_WRITE_EFFECTIVE_MTU` pattern for its own Write direction (docs/LESSONS.md's "att-mtu-vs-attribute-length" — the real cap is the 64-byte Notify characteristic, not the raw MTU). Resets to `FEB_DEFAULT_ATT_MTU` (23) on `profile_start()`. Build-verified clean (FAP + 527/527 host tests). **Needs a live test**: confirm MTU negotiation completes and fragment count actually drops for `hello_ack`/wardriving/scan records on real hardware. |

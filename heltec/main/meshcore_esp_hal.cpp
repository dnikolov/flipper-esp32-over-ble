// meshcore_esp_hal.cpp
// Definitions for the vendored ESP-IDF HAL -- see meshcore_esp_hal.h's top comment for why
// this is vendored here instead of coming from the jgromes/radiolib managed component.

/* This project's heltec/sdkconfig.defaults uses ESP-IDF's default CONFIG_FREERTOS_HZ=100
   (10ms tick) -- shared across every task in this firmware, not something this one
   capability should change unilaterally (a global tick-rate change would affect every other
   task's timing assumptions, untested here). The upstream check below exists because a 10ms
   tick makes LoRaWAN's timing-critical RX1/RX2 receive windows unreliable; this capability
   never does that (or any TX at all) -- it only ever runs RadioLib's one-time begin()/
   startReceive() setup calls (whose own internal delay()s are configuration-settling waits,
   not receive-window deadlines) followed by indefinite passive continuous receive. Opting
   out here is a deliberate, scoped judgment call for this specific (RX-only, no LoRaWAN)
   use case, not a blanket "the check doesn't matter" dismissal. */
#define RADIOLIB_RELAX_RTOS_TICK 1

/* Location-source interface (docs/PLAN.md "Real GPS driver, wardriving fix-dependency, and
   ...
   2026-09-23 per docs/hardware/heltec-wifi-lora-32-v2/README.md's GPIO17-wired GPS module).
   Backed by a real UART/NMEA-0183 GPS module (location.c opens a UART port and runs a
   dedicated background parse task built on nmea_parser.c/.h's pure sentence parsing).
   Consumed only by the `gps` capability's handle_gps_command() here -- wardriving has not
   been ported to this board, so there is no record-capture call site yet. Identical shape to
   esp32/main/location.h; only location.c's compile-time pin/UART-port constants differ per
   board. */
...
/* Opens the GPS UART (this board: UART1, RX=GPIO17, TX=GPIO23 configured-but-unused, 9600
   8N1, no flow control -- compile-time constant, see location.c) and starts a dedicated
   background parse task, RX-only (never transmits to the module). A UART/task-start failure
   is logged and leaves the driver permanently in FEB_LOCATION_NO_SIGNAL -- GPS is optional
   peripheral hardware, not required for the board to boot. */

...
/* docs/PLAN.md "Real GPS driver, wardriving fix-dependency, and real wardriving-record
   ...
   2026-09-23. The GPS module is the same one bench-tested on GPIO36 (see
   docs/hardware/heltec-wifi-lora-32-v2/README.md's now-superseded GPIO36 note) and has since
   been rewired by the user to GPIO17 for the real firmware port -- GPIO17 does not appear in
   that doc's OLED/LoRa/I2C/UART0/LED/boot-button pin tables and is not one of this board's
   classic-ESP32 strapping pins (GPIO0/2/4/5/12/15), so it carries no boot-mode risk. UART_NUM_1
   is unclaimed elsewhere in this firmware (checked heltec's other .c files). TX_GPIO (GPIO23)
   is likewise unclaimed and non-strapping; it is configured only so the UART peripheral's TX
   signal isn't left routed to an undefined pin, and is never written to (RX-only, matching
   esp32/main/location.c's convention -- an earlier wake/cold-start command burst was proven
   actively harmful in that board's antenna smoke test). Runtime-configurable pins are
   deliberately out of scope, same as the C6 (docs/PLAN.md's scope boundary). */
static const char *TAG = "feb_location";

#define FEB_GPS_UART_PORT UART_NUM_1
#define FEB_GPS_UART_RX_GPIO 17
#define FEB_GPS_UART_TX_GPIO 23
...
/* Own dedicated stack (docs/BACKLOG.md's cost-efficiency section flags this project's
   ...
   is static (below), not stack-resident. 3072 bytes matches esp32/main/location.c's precedent
   and this codebase's existing lightweight-task sizing (main.c's reconnect_task,
   factory_reset.c's factory_reset_task). */
